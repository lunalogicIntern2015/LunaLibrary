...
#include <PDE/Scheme/Scheme_HV.h>
...
Scheme_HV :: Scheme_HV(
				  ...
				  Y_prime   (matrix_size.first+2,matrix_size.second+2,0)
				  {
				   //   if(lambda_ != 0.5)
					  //{
						 // throw ("Error in constructor of Scheme_HV, lambda != 0.5 doesn't make sense :) ");
					  //}
				  }
...
//! ----------------------------------------------------
//! scheme_0
void Scheme_HV :: initializeScheme_0_x(double t1, double t2, int index_y, Matrix& U_0)
{
	//B_x_tri_
...
void Scheme_HV :: initializeScheme_0_y(double t1, double t2, int index_x, Matrix& U_0)
{
	//B_y_tri_
...
void Scheme_HV:: calculate_one_step_0 (int index_t_n, Matrix& U_0) 
...
void Scheme_HV :: initializeScheme_1(double t1, double t2, int index_y, Matrix& U_0)
{
...
void Scheme_HV::calculate_one_step_1 (int index_t_n, int index_y, Matrix& U_0) // X direction:  (1-theta*delta_t*L_x)Y_1      = Y_0 - (theta*deltat_t*L_x)U_0 
{
	double t1      = discret_.get_discret_t(index_t_n);
	double t2      = discret_.get_discret_t(index_t_n+1);
...
void Scheme_HV :: initializeScheme_2(double t1, double t2, int index_x, Matrix& U_0)
{
	double scalar = -theta_*(t2-t1); //delta_t_;
...
void Scheme_HV::calculate_one_step_2 (int index_t_n, int index_x, Matrix& U_0) // Y direction:  (1-theta*delta_t*L_y)U_result = Y_1 - (theta*deltat_t*L_y)U_0
{
	double t1      = discret_.get_discret_t(index_t_n);
	double t2      = discret_.get_discret_t(index_t_n+1);
...
//! ----------------------------------------------------
//! scheme_0_prime
void Scheme_HV :: initializeScheme_0_prime_x(double t1, double t2, int index_y, Matrix& U_0)
{
	//B_x_tri_
	construct_L_x_n(t1, index_y);
	double scalar = 0.5*(t2-t1); //delta_t_;
	B_x_tri_ = L_x_n_*scalar;
	A_x_tri_ = TridiagonalMatrix(matrix_size.first, "id");
...
void Scheme_HV :: initializeScheme_0_prime_y(double t1, double t2, int index_x, Matrix& U_0)
{
	//B_y_tri_
	construct_L_y_n(t1, index_x);
	double scalar = 0.5*(t2-t1); //delta_t_;
...
void Scheme_HV :: calculate_one_step_0_prime (int index_t_n, Matrix& U_0) 
// Total explicit: Y_0_prime = Y_0 + 0.5*L_0(Y_2 - U_0) + 0.5*(L_1+L_2)(Y_2-U_0), do it on direction x
{
	double t1  = discret_.get_discret_t(index_t_n);
	double t2  = discret_.get_discret_t(index_t_n+1);
	...
	Y_prime = Y_2 - U_0; 
	caluclate_explicit_cross_derivative(t1, Y_prime);
	double scalar = 0.5*(t2-t1); //delta_t_;
	...
	//! New MCS part:
	// ---- (1/2 - theta)(L_1+L_2)(Y_2-U_0) ---- 
	//! Don't care about the boudnary condition ! 
	//! x direction for: L_x_n_
	for(int index_y = 1; index_y<matrix_size.second+2-1; ++index_y)
	{
		//! U^n_x: copy index_y'eme column of U --> U_x
		copy_matrixColumnToVector_withoutFirstAndLastElment(Y_prime,U_1_x,index_y); // without 1st & last element

		//! Construct A,B,F,G
		initializeScheme_0_prime_x(t1, t2, index_y, Y_prime);  //! only explicit use Y_prime, if not can use U_0 for approximation ...
	...
	for(int index_x = 1; index_x<matrix_size.first+2-1; ++index_x)
	{
		...
		//! UplusHalf_y: copy index_x'eme row of UplusHalf --> UplusHalf_y  
		copy_matrixRowToVector_withoutFirstAndLastElment(Y_prime, U_1_y, index_x);

		//! Construct A,B,F,G
		initializeScheme_0_prime_y(t1, t2, index_x, Y_prime); //! ??? what to use 
...
void Scheme_HV :: calculate_one_step_1_prime (int index_t_n, int index_y, Matrix& U_0) // X direction:  (1-theta*delta_t*L_x)Y_1      = Y_0 - (theta*deltat_t*L_x)U_0 
{
	double t1      = discret_.get_discret_t(index_t_n);
	double t2      = discret_.get_discret_t(index_t_n+1);
	...
	{ 
		//! U^n_x: copy index_y'eme column of U --> U_x
		copy_matrixColumnToVector_withoutFirstAndLastElment(Y_2,U_1_x,index_y); // without 1st & last element
...
void Scheme_HV :: calculate_one_step_2_prime (int index_t_n, int index_x, Matrix& U_0) // Y direction:  (1-theta*delta_t*L_y)U_result = Y_1 - (theta*deltat_t*L_y)U_0
{
	double t1      = discret_.get_discret_t(index_t_n);
	double t2      = discret_.get_discret_t(index_t_n+1);
	...
	{
		//! UplusHalf_y: copy index_x'eme row of UplusHalf --> UplusHalf_y  
		copy_matrixRowToVector_withoutFirstAndLastElment(Y_2, U_1_y, index_x);
		...
		Y_2[index_x][0]			    = bc_->bc_Y_L_->get_bc_extremValue(t1, t2, index_x, y_left_border_index,  U_0);
		Y_2[index_x][U_0.cols()-1]  = bc_->bc_Y_R_->get_bc_extremValue(t1, t2, index_x, y_right_border_index, U_0);
	}
}
...
//! Final 
void Scheme_HV :: calculate_one_step(int index_t, Matrix& U_0) //, Matrix& U_1, Matrix& U_2)
{
		...
		for(int index_y = 0; index_y<matrix_size.second+2; ++index_y)
		{
			calculate_one_step_1_prime(index_t, index_y, U_0);
		}
		...
		{
			//! ATTENTION Uplus <--> U exchang role
			calculate_one_step_2_prime(index_t,  index_x, U_0);
		}
