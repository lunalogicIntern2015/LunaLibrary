...
#include <PDE/Scheme/Scheme_MCS.h>
...
//! ----------------------------------------------------
//! scheme_0
void Scheme_MCS :: initializeScheme_0_x(double t1, double t2, int index_y, Matrix& U_0)
{
	//B_x_tri_
...
void Scheme_MCS :: initializeScheme_0_y(double t1, double t2, int index_x, Matrix& U_0)
{
	//B_y_tri_
...
void Scheme_MCS:: calculate_one_step_0 (int index_t_n, Matrix& U_0) 
...
void Scheme_MCS :: initializeScheme_1(double t1, double t2, int index_y, Matrix& U_0)
{
...
void Scheme_MCS::calculate_one_step_1 (int index_t_n, int index_y, Matrix& U_0) // X direction:  (1-theta*delta_t*L_x)Y_1      = Y_0 - (theta*deltat_t*L_x)U_0 
{
	double t1      = discret_.get_discret_t(index_t_n);
	double t2      = discret_.get_discret_t(index_t_n+1);
...
void Scheme_MCS :: initializeScheme_2(double t1, double t2, int index_x, Matrix& U_0)
{
	double scalar = -theta_*(t2-t1); //delta_t_;
...
void Scheme_MCS::calculate_one_step_2 (int index_t_n, int index_x, Matrix& U_0) // Y direction:  (1-theta*delta_t*L_y)U_result = Y_1 - (theta*deltat_t*L_y)U_0
{
	double t1      = discret_.get_discret_t(index_t_n);
	double t2      = discret_.get_discret_t(index_t_n+1);
...
//! ----------------------------------------------------
//! scheme_0_prime
void Scheme_MCS :: initializeScheme_0_prime_x(double t1, double t2, int index_y, Matrix& U_0)
{
	//B_x_tri_
	construct_L_x_n(t1, index_y);
	double scalar = (0.5-theta_)*(t2-t1); //delta_t_;
	B_x_tri_ = L_x_n_*scalar;
	A_x_tri_ = TridiagonalMatrix(matrix_size.first, "id");
...
void Scheme_MCS :: initializeScheme_0_prime_y(double t1, double t2, int index_x, Matrix& U_0)
{
	//B_y_tri_
	construct_L_y_n(t1, index_x);
	double scalar = (0.5-theta_)*(t2-t1); //delta_t_;
...
void Scheme_MCS:: calculate_one_step_0_prime (int index_t_n, Matrix& U_0) 
// Total explicit: Y_0_prime = Y_0 + lambda*L_0(Y_2 - U_0) + (1/2 - theta)(L_1+L_2)(Y_2-U_0), do it on direction x
{
	double t1  = discret_.get_discret_t(index_t_n);
	double t2  = discret_.get_discret_t(index_t_n+1);

	//! Same as CS
	// ---- Y_0 + lambda*L_0(Y_2 - U_0) ---- 
	...
	for(int index_y = 1; index_y<matrix_size.second+2-1; ++index_y)
	{
		...
		//! Construct A,B,F,G
		initializeScheme_0_prime_x(t1, t2, index_y, Y_2);  //only explicit, BC use Y
	...
	for(int index_x = 1; index_x<matrix_size.first+2-1; ++index_x)
	{
		...
		//! Construct A,B,F,G
		initializeScheme_0_prime_y(t1, t2, index_x, Y_2); //! only explicit: use Y
...
//! Final 
void Scheme_MCS :: calculate_one_step(int index_t, Matrix& U_0) //, Matrix& U_1, Matrix& U_2)
{
