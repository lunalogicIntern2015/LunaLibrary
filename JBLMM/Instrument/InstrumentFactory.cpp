...
GeneticSwap_CONSTPTR InstrumentFactory::createVanillaSwap(	 double strike, 
															 ...
															 double nominal)
	{
		...
		for(size_t i=0;i<floatingLegPaymentIndexSchedule.size(); i++)	//
		{
			...
			Rate_CONSTPTR liborRate(new LiborRate(liborFixingindex, leg1Tenor));
		...
		for(size_t i=0;i<fixedLegPaymentIndexSchedule.size(); i++)
		{
			...
			Rate_CONSTPTR constRate(new ConstRate(strike));
		...
		GeneticSwap_CONSTPTR vanillaSwap(	new GeneticSwap(	CouponLeg_CONSTPTR(new CouponLeg(floatingCouponVector)),
																CouponLeg_CONSTPTR(new CouponLeg(fixedCouponVector))));
		return vanillaSwap;
	}


GeneticSwap_CONSTPTR InstrumentFactory::createStandardTARNSwap(	double strike, 
															LMM::Index indexStart, 
												 			LMM::Index indexEnd, 
															Tenor leg1Tenor,		//
															Tenor leg2Tenor,		
															...
															double target)
{
	...
	GeneticSwap_CONSTPTR targetSwap(	new GeneticSwap(	CouponLeg_CONSTPTR(new CouponLeg(leg1CouponVector)),
															CouponLeg_CONSTPTR(new CouponLeg(leg2CouponVector))));
	return targetSwap;
}


GeneticSwap_CONSTPTR InstrumentFactory::createGeneticSwap(CouponLeg_CONSTPTR Leg1, CouponLeg_CONSTPTR Leg2)
{
	GeneticSwap_CONSTPTR geneticSwap(new GeneticSwap(Leg1,Leg2));
	return geneticSwap;
}
