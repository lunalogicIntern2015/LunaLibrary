...
double McGeneticSwapLMMPricer::swapNPV(const GeneticSwap_CONSTPTR geneticSwap, size_t nbSimulation)const
{
	...
	for(size_t itrSimulation=0; itrSimulation<nbSimulation; ++itrSimulation)
	{
		...
		double npv1  = evaluateCouponLeg(indexValuationDate, geneticSwap->getLeg1(), mcLmm_->get_numeraire(), mcLmm_->get_liborMatrix(),lmmTenorStructure);
		double npv2  = evaluateCouponLeg(indexValuationDate, geneticSwap->getLeg2(), mcLmm_->get_numeraire(), mcLmm_->get_liborMatrix(),lmmTenorStructure);
...
double McGeneticSwapLMMPricer::evaluateCouponLeg(	const LMM::Index indexValuationDate,
...
double McGeneticSwapLMMPricer::evaluateCappedFlooredCoupon(CappedFlooredCoupon_CONSTPTR cappedFlooredCoupon, double rateValue)const
{
