...
class CheyetteMarketData	
{
private:
	CourbeInput_PTR		pCourbeInput_ ;
...
public:
	//TODO constructeur avec un fichier

	//constructeur 2
	CheyetteMarketData(	CourbeInput_PTR	pCourbeInput, 
						std::vector<size_t> a_expiry, std::vector<size_t> b_tenor, std::vector<double> strikeATM, 
		...
		:	pCourbeInput_(pCourbeInput), a_expiry_(a_expiry), b_tenor_(b_tenor), strikeATM_(strikeATM), 
	...
	CourbeInput_PTR		getCourbeInput() const {return pCourbeInput_ ;}
...
typedef boost::shared_ptr<CheyetteMarketData>       CheyetteMarketData_PTR;
typedef boost::shared_ptr<const CheyetteMarketData> CheyetteMarketData_CONSTPTR;
