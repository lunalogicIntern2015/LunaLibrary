...
double CheyetteDD_Model::sigma_r( double t,  double x_t, double y_t) const 
{
	double sigma_t = cheyetteDD_Parameter_.sigma_(t) ;
	double m_t		= cheyetteDD_Parameter_.m_(t) ;
	double shift1	= shift1_->operator()(t, x_t, y_t) ;
	double shift2	= shift2_->operator()(t, x_t, y_t) ;  //en pratique pointe vers shift1(0, 0)
	return sigma_t * (m_t * shift1 + (1 - m_t) * shift2) ;
}

double CheyetteDD_Model::sigma_r_t_1stDerivative( double t,  double x_t, double y_t) const 
{
	double sigma_t = cheyetteDD_Parameter_.sigma_(t) ;
	double m_t		= cheyetteDD_Parameter_.m_(t) ;
	return sigma_t * (m_t * derivative_x_shift1_->operator()(t, x_t, y_t) 
					+ (1 - m_t) * derivative_x_shift2_->operator()(t, x_t, y_t) )  ;
}

//shift functions

//y_t pas utile ms fonction R3 -> R
double CheyetteDD_Model::shift_rt(double t,  double x_t, double y_t) const   
{
	return x_t + courbeInput_PTR_->get_f_0_t(t) ;  
}	

double CheyetteDD_Model::shift_f_0_t(double t,  double x_t, double y_t) const
{
	return courbeInput_PTR_->get_f_0_t(t) ;  
}				

//derivee du shift wrt x_t
double CheyetteDD_Model::shift_rt_1stDerivative(double t,  double x_t, double y_t) const
{
	return 1. ;
}
double CheyetteDD_Model::shift_r0_1stDerivative(double t,  double x_t, double y_t) const
{
	return 0. ; 
} 
double CheyetteDD_Model::shift_f_0_t_1stDerivative(double t,  double x_t, double y_t) const
{
	return 0. ;
}
