...
void McSpotLmm::SpotLmmModel_Euler()
{
	...
	{ 
		...
		for(size_t k = indexBeta; k<=horizon_; ++k)  // k : is a Libor index
		{
			...
			double shift_k = lmm_->get_shift_timeIndexVersion(k,indexTime/*,liborsInitValue_*/);
		...
		{ 		 
			...
			double equivalentShift = lmm_->get_shift_timeIndexVersion(indexLibor,indexTime/*,liborsInitValue_*/);
...
void McSpotLmm::SpotLmmModel_Pc()
{
	...
	{ 
		...
		{
			double libor_k = liborMatrix_(k,indexTime-1);
			double shift_k = lmm_->get_shift_timeIndexVersion(k,indexTime/*,liborsInitValue_*/); // YY no idear: pc shceme should use the same or different shift (for time diff).
			double delta_k = deltaT[k];
			drift_coeff[k] = delta_k*(libor_k+shift_k) / (1+ delta_k*libor_k); // Positive drift !
		}
