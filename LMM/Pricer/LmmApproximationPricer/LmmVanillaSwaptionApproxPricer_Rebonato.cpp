...
void LmmVanillaSwaptionApproxPricer_Rebonato::update_Correlation()
{
	lmm_->update_Correlation();
	initializeConstShift();
}
...
double LmmVanillaSwaptionApproxPricer_Rebonato::volBlack(const VanillaSwaption& vanillaswaption, const std::vector<double>& liborsInitValue) const
{
	...
	for(size_t i=0; i<floatingLegPaymentIndexSchedule.size(); ++i)
	{
		...
		for(size_t j=0; j<floatingLegPaymentIndexSchedule.size(); ++j)
		{
			...
			const double cumulated_covariance      = lmm_->get_cumulatedcovarianceTensor(swaptionIndexMaturity, indexLibor_i, indexLibor_j); //L_i, L_j 's integral of vol in [T_0,T_k], T_k =swaptionIndexMaturity
			const double phi_i = buffer_liborAtExpensionInit[indexLibor_i] + constShifts_[indexLibor_i]; 
			const double phi_j = buffer_liborAtExpensionInit[indexLibor_j] + constShifts_[indexLibor_j];
			const double omega_i = buffer_omegaAtExpensionInit[indexLibor_i];
			const double omega_j = buffer_omegaAtExpensionInit[indexLibor_j];
			
			volSquare += omega_i*omega_j*phi_i*phi_j*cumulated_covariance; 
		}
	}
